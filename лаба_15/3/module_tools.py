...
# Функция для получения элемента их файла по индексу
def get_element(file, index):
    file.seek(index * 4)
    num = unpacking_line(file.read(get_line_size()))
    return num


# Функция для перестановки соседних элементов
def swap_elements(f, index_1, index_2):
    element_1 = get_element(f, index_1)
    element_2 = get_element(f, index_2)
    f.seek(index_1 * get_line_size())
    f.write(struct.pack('>i', element_2))
    f.seek(index_2 * get_line_size())
    f.write(struct.pack('>i', element_1))
